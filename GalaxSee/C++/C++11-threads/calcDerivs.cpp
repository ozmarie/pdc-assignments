 ...
 * 
 * HINT: although this project focuses on using the C++11 thread library to solve the problem,
 *          I recommend using OpenMP's operations for critical/atomic operations.
 */
 ...
 */
void NbodyModel::calcDerivs(double *x, double *derivs, double t, double tStep)
{
    ...
    for (unsigned i = 0; i < num_bodies; ++i) {
        ...
        for (unsigned j = i + 1; j < num_bodies; ++j)
        {
            // Compute the softened radius for gravitational force calculation
            double srad = computeSoftenedRadius(G * mass[j], tStep);
            ...
            if (i != j)
            {
                ...
                if (rad > srad)
                {
                    ...
                    double grav_effect = G * inv_rad3;

                    // Update the acceleration components due to body j
                    derivs[i * 6 + 3] += grav_effect * mass[j];
                    derivs[i * 6 + 4] += grav_effect * mass[j];
                    derivs[i * 6 + 5] += grav_effect * mass[j];

                    // Apply Newton's third law to update the acceleration of body j
                    derivs[j * 6 + 3] -= grav_effect * mass[i];
                    derivs[j * 6 + 4] -= grav_effect * mass[i];
                    derivs[j * 6 + 5] -= grav_effect * mass[i];
                }
            }
        }
    }
 ...
 */
double NbodyModel::computeSoftenedRadius(double g_m, double tstep)
{
    // g_m = G * mass;
