...
#pragma acc routine(lcrng_initialize)
#pragma acc routine(lcrng_random)
#pragma acc routine(lcrng_anbn)
#pragma acc routine(congruence)
#pragma acc routine(i4_gcd)
#pragma acc routine(i4_max)
#pragma acc routine(i4_min)
#pragma acc routine(i4_sign)
#pragma acc routine(lcrng_evaluate)
#pragma acc routine(power_mod)
/******************************************************************************/
/*
  LCRNG
  Purpose:
...
/******************************************************************************/
/* Documentation for a (removed) main() function
  Purpose:
...
/*
  Purpose:
    ...
    LCRNG_ANBN computes the "N-th power" of a linear congruential generator.

  Discussion:

    We are considering a linear congruential random number generator.
    The LCRNG takes as input an integer value called SEED, and returns
    ...
    In most cases, a user is content to call the LCRNG repeatedly, with
    ...
    that describe the LCRNG that is equivalent to N applications of the
    original LCRNG.
    ...
    by starting with a single value of SEED, using the original LCRNG to generate
    the first N-1 "iterates" of SEED, so that you now have N "seed" values,
    and from now on, applying the N-th power of the LCRNG to the seeds.
    ...
    original LCRNG directly if we can solve
  ...
  Parameters:
    ...
    Input, int A, the multiplier for the LCRNG.

    Input, int  B, the added value for the LCRNG.
    ...
    LCRNG is to be applied.  It is required that 0 <= N.

    Output, int *AN, *BN, the multiplier and added value for
    the LCRNG that represent N applications of the original LCRNG.
*/
{
  int am1;
  int anm1tb;
...
/*
  Purpose:
    ...
    LCRNG_EVALUATE evaluates an LCRNG, y = ( A * x + B ) mod C.
  ...
  Parameters:
    ...
    Output, int LCRNG_EVALUATE, the processed value.
*/
{
  long long int a8;
  long long int b8;
...
// The following two functions were added by Steven McKelvey, Calvin University, August 2024
// Functions to abstract the details of this module:
lcrng lcrng_initialize(int numProcs, int id)
{
  lcrng myrng;

  // values from example found at https://people.math.sc.edu/Burkardt/c_src/random_mpi/random_mpi.html
  myrng.a = 16807;
  myrng.b = 0;
  myrng.c = 2147483647;

  // initialize an and bn for all processes:
  lcrng_anbn(myrng.a, myrng.b, myrng.c, numProcs, &myrng.an, &myrng.bn);

  // set up random number generator (process-specific values)
  myrng.currentRandom = 12345; // value from example found at https://people.math.sc.edu/Burkardt/c_src/random_mpi/random_mpi.html

  // run this loop until the process's id is reached (when we have found this process's starting value).
  // this generates the initial random number for each thread.
  for (int j = 1; j <= id; ++j)
  {
    int prevRand = myrng.currentRandom;
    myrng.currentRandom = lcrng_evaluate(myrng.a, myrng.b, myrng.c, prevRand);
  }
  return myrng; // return the newly initialized random number generator
}

/* lcrng_random()
 * Generates and returns a random number and updates the lcrng object
 */
int lcrng_random(lcrng *rng)
{
  // set the next "currentRandom" (the next state) to be whatever is returned in this function.
  rng->currentRandom = lcrng_evaluate(rng->an, rng->bn, rng->c, rng->currentRandom);
  return rng->currentRandom;
}
