...
#include <string.h>
#include "X-graph.cuh"
#include "display.cuh" /* getDisplay() */

void xgraphSetup(xgraph *thegraph, int width_in, int height_in, string name)
{
    ...
    thegraph->buffer =
        XCreatePixmap(thegraph->dpy, DefaultRootWindow(thegraph->dpy),
                      thegraph->width, thegraph->height, DefaultDepth(thegraph->dpy, DefaultScreen(thegraph->dpy)));
    ...
    XStoreName(thegraph->dpy, thegraph->w, name.c_str());
