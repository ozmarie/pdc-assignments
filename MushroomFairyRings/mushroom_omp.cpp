...
// A simulation of mushroom fairy rings using a grid-based approach.
//
...
int main(int argc, char *argv[]) {
    ...
    int animation = 0;
    // whether to place the initial spore in the center of the grid
    int centerSpore = 0;
    ...
    getArguments(argc, argv, &width, &length, &itEnd, &debug, &graphics,
                 &animation, &movie, &centerSpore, &numThreads);
    ...
    printf("centerSpore %d   movie %d   animation %d\n", centerSpore, movie, animation);
    ...
    if (graphics || movie) {
        ...
        // drawFirstAndLast(width, length);  // draw the initial and final grids
...
void initCenterGrid(int *grid, int w, int l) {
    int i, j;
    // Initialize the grid with a spore in the center
    for (i = 1; i <= l; i++) {
        for (j = 1; j <= w; j++) {
            if ((i == l / 2) && (j == w / 2)) {
                grid[i * (w + 2) + j] = 1;  // SPORE
            } else {
                grid[i * (w + 2) + j] = 0;  // EMPTY
            }
        }
    }
}
